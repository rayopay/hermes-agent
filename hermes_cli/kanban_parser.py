...
_SPECS = [
    ...
    _cmd("unblock", [
        _arg("--recovery-json", help="Structured recovery object (action=classify|retry|resolved_resume|finalize). Release/finalize requires blocker_id, initiation_mode, settlement_refs; user_directed also requires instruction_ref. Show --json supplies a read-only settlement_scope.template: owner must assess it before adding assertion=settled and reference. Exactly one task; "
             "requires observed_token, block_event_id, rationale, evidence_refs; classify alone accepts existing_blocker_id. "
             "Classification stays held; release returns actual scheduling status, never launches. Finalize completes tracked settled triage directly to done only for an already-bound exact GitHub PR contract with current native acceptance and no scratch workspace. References attribute the request, not acceptance proof. Errors may follow committed done; inspect status, never replay. Cannot combine with --reason."),
