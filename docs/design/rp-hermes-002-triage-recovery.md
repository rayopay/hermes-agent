...
## Pinned-main integration checkpoint (bounded qualification verified)

The feature-branch base-integration merge combines feature `c91dc9277e803b1d349621a81f989e359d204c59` with pinned main `36f2e2ec916a97f960bfb356f92eb549d5e58621` (merge base `d77d61287012a53fe915c11e950bbcc72a0a7630`). This is not a merge of PR #6 into main. RP-HERMES-003 / PR #5 is merged in that base, not thereby deployed; RP-HERMES-001 / PR #3 remains open and is not integrated.

**Current finalization acceptance is reported-CI**, as defined in the [RP-HERMES-003 decision](../decisions/RP-HERMES-003-reported-ci-acceptance.md): exact current PR head, at least one success, skipped/neutral only alongside success, and every reported failure blocks, including optional failures. No required-policy discovery or replacement configuration is introduced. App/suite identity, pagination completeness, duplicate rejection, final head/base/state revalidation, exact persisted PR binding and local-only no-network behavior remain native safeguards. Missing expected checks that never report cannot be detected.

The increment sections below are **historical checkpoint evidence**, not qualification of this merged candidate. In particular, the native-finalization increment's configured-required-check description records its old base, not current acceptance. The unchanged legacy fixture initially failed before completion because its REST PR omitted `merged`; race/SQL/postcommit seams were not reliably reached. Integration corrects only the shared transport fixture and adds native SQLite composition controls, leaving the incoming collector and acceptance tests intact. Final integration, provider-normalized recovery schemas, executable CLI qualification, environment completeness, exact-head review/CI and deployment remain separate gates; no combined RP-001 readiness is claimed.

Bounded merged-candidate evidence: **716 passed, zero failed across 43 distinct files**, canonical two-worker runner with file retries disabled, actual runner exit 0. All prior 29 files are retained. Ten new native finalization composition cases cover optional reported failures, success with skipped/neutral outcomes, no successful evidence, malformed identities and head/base movement. Positive cases verify exact `reported-ci` receipts and preserved history/dependencies; explicit race and SQL-fault entry assertions prevent an unrelated earlier refusal from masquerading as proof. The original 42-pass/16-failure fixture incompatibility and subsequent 58-pass/10-setup-failure wrong-table fixture run remain recorded separately.

Independent specification and quality reviews approved this bounded reconciliation. All incoming-only endpoints, including deleted/renamed endpoints, match pinned main; the four executable overlap files retain their automatic merge bytes. Reconciliation edits only documentation and two test files, not production policy. Source hashes and 85 distribution name/version/location metadata records were stable during the final run. Recovery fixtures use controlled external transport with the real native acceptance engine; neither this result nor generic schema coverage certifies recovery-specific provider normalization, executable CLI behavior, live GitHub, complete dependencies, or the full upstream suite.

## Provider-schema and executable-refusal qualification

Base checkpoint: `bfa8747f49c57172eb53957ef84329ffa21e857c`. This increment adds only `evals/kanban_recovery_cli.py`, `tests/agent/test_kanban_recovery_provider_schemas.py` and `tests/evals/test_kanban_recovery_cli.py`, plus evidence documentation; production code is unchanged. Independent specification and quality rereviews approved the corrected bounded checkpoint.

The latest canonical two-worker/no-retry union records **738 passed, zero failed across 45 files**, actual runner and launcher exits 0. All prior 716/43 cases remain; five real provider-converter cases and 17 launcher rejection cases extend them. Source hashes and 85 package name/version/location records remained unchanged within the final run; this is not package-byte, complete-environment or full-suite proof.

Provider tests exercise the actual Responses, Anthropic, Moonshot and both Gemini conversion paths, recursively checking the registered recovery schema's types, required fields, enums and constraints. Responses explicitly retains `strict=False`; legacy Gemini's removed property restrictions are explicitly accounted for. These are local converter/dispatch checks, not live-provider inference or `strict=True` compatibility.

The candidate CLI harness requires a literal validated `--expected-head`, passed unchanged to children, and retains fleet-specific repository/interpreter/entrypoint/dotenv pins before accepted execution or scratch allocation. Rejection-only tests use the canonical test process's `sys.executable`. Invalid inventory and inherited-authority refusals precede private repository validation. Real ordinary-checkout tests without `.venv` reproduce the original ordering defect (**15 passed / 2 failed**) and pass after correction; an otherwise-valid request still fails the repository pin. Seventeen independent guard receipts corroborate this rejection-only scope. Empty stdout is output evidence, not filesystem confinement proof.

Actual delegated CLI subprocesses reject init and all four recovery actions at the CLI-entry authority fence. **Owner-positive lifecycle remains BLOCKED / UNEXECUTED for both parent and child** under inherited delegation authority. The staged positive sequence has been statically reviewed, including new-identity classification, consumed-resolution-evidence refusal and explicit parent-held retry, but not executed. Never remove authority markers, directly invoke the internal child-root interface or substitute another route to manufacture owner authority. Source approval does not authorize owner execution. Linux rejection coverage does not establish accepted ordinary-checkout or non-Linux lifecycle support; trusted internal child-root and timeout/descendant-settlement limitations remain explicit.

Hosted CI on the base checkpoint separately reports failures in Python tests, blocking Ruff enforcement and Windows-footgun checks ([run 35051613810](https://github.com/rayopay/hermes-agent/actions/runs/35051613810)). The following settlement correction records the subsequent diagnosis; local bounded results do not waive hosted failures. Owner-positive executable qualification, final integration/release assessment, fresh exact-head CI/review and deployment remain outstanding. No installed runtime, profile or live-card state changed.

## Settlement CI correction: verified evidence

Base: `7af542988852fec5268b842fa3bc5b78fa273875`. Production changes are limited to an explicit UTF-8 boot-ID read and replacing the unconditional application-level `os.kill(pid, 0)` probe with already-pinned core `psutil.pid_exists`. Only its exact `False` return permits progression. Invalid PIDs, live/reused PIDs, zombies, unexpected return values and the specified psutil/OS/value/type exceptions retain the hold. Both release and unchanged direct finalization use this shared seam. The Linux boot-ID prerequisite remains unchanged; this does not add Windows lifecycle support or prove absence of detached/remote processes.

Canonical no-retry qualification: **779 passed, zero failed across 47 files**, actual exit 0, retaining all prior 738/45 cases. Two new parametrized invariants contribute 40 cases and the existing repository scanner contributes one. Both native consumers preserve full SQLite state and avoid acceptance transport on denial. Real pipe-controlled owned processes demonstrate live and unreaped-zombie holds, followed by accepted progression only after reaping and confirmed PID absence. Causal red **16 passed / 25 failed** is retained: 24 direct-probe sentry failures and the scanner failure. This is not 24 independent red demonstrations of every exception policy.

The full-repository Windows-footgun scanner reports no findings across 1,568 files; targeted Ruff passes. All 85 package metadata records and within-run source hashes were stable. Independent specification PASS and quality APPROVED apply to this exact correction. An initial receipt-collection failure caused by runner temporary-root cleanup is preserved; an unchanged-source retained-basetemp **40/40** rerun recovered owned-process receipts without increasing the 779 total. Successful owned-child reaping is evidenced, not cleanup under every timeout or exhaustive descendant settlement. Abnormal-timeout cleanup remains an optional test robustness improvement.

Historical CI diagnosis at `bfa8747f49c57172eb53957ef84329ffa21e857c` reconciled **49,740 passed / 8 final failures**: the scanner failure reflects the two corrected downstream findings; seven other final failures have independent evidence on pinned main (six final failures plus a browser-threshold failure before a successful baseline retry). A compression flake is also baseline-evidenced; the completed-process follow-up flake has unknown baseline provenance. These failures are preserved, not repaired or waived. The later `7af542988852fec5268b842fa3bc5b78fa273875` hosted run also reports failed Python and blocking lint jobs; the older detailed attribution is not qualification of that newer run. Fresh exact-head hosted CI/review remains outstanding. Owner-positive CLI remains **BLOCKED / UNEXECUTED** for both parent and child; no merge, deployment or live-card recovery is authorized by this checkpoint.
